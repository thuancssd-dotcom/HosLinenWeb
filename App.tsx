...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      const isOfficial = localStorage.getItem('hosplinen_is_official_v1') === 'true';
      if (raw) {
        const parsed = JSON.parse(raw);
        let loadedItems = parsed.DV;
        let loadedDetail = parsed.DETAIL;
        let loadedDepts = parsed.DEPARTMENTS || DEPARTMENTS;

        // Force upgrade to official numbers if not already upgraded
        if (!isOfficial) {
          loadedItems = INITIAL_LINEN_ITEMS;
          loadedDetail = INITIAL_DETAIL_ALLOCATIONS;
          loadedDepts = DEPARTMENTS;
          localStorage.setItem('hosplinen_is_official_v1', 'true');
          
          // Save back right away to commit the official update
          const payload = {
            ...parsed,
            DV: INITIAL_LINEN_ITEMS,
            DETAIL: INITIAL_DETAIL_ALLOCATIONS,
        ...
        if (Array.isArray(loadedItems)) setItems(loadedItems);
        if (loadedDetail) setDetailAllocations(loadedDetail);
        if (Array.isArray(loadedDepts)) {
        ...
        if (Array.isArray(parsed.ROLES)) {
          const migratedRoles = parsed.ROLES.map((r: any) => {
            const perms = r.perms || {};
            const dovai = perms.dovai !== undefined ? perms.dovai : (r.name !== 'Kế toán' && r.name !== 'Xem báo cáo');
            ...
            if (nameLower.includes('hộ lý')) {
              return {
                ...
                perms: {
                  nhap: perms.nhap !== undefined ? perms.nhap : false,
                  thuhoi: perms.thuhoi !== undefined ? perms.thuhoi : false,
                  xuat: perms.xuat !== undefined ? perms.xuat : false,
                  huy: perms.huy !== undefined ? perms.huy : false,
                  dc: perms.dc !== undefined ? perms.dc : false,
                  dovai: dovai
                }
              };
            }
            if (nameLower.includes('điều dưỡng')) {
              const lockedV1 = localStorage.getItem('dovai_locked_v1');
              const finalDovai = !lockedV1 && dovai !== false ? false : dovai;
              if (!lockedV1) localStorage.setItem('dovai_locked_v1', 'true');
              return {
                ...r,
                perms: {
                  ...perms,
                  nhap: perms.nhap !== undefined ? perms.nhap : false,
                  thuhoi: perms.thuhoi !== undefined ? perms.thuhoi : true,
                  xuat: perms.xuat !== undefined ? perms.xuat : false,
                  huy: perms.huy !== undefined ? perms.huy : false,
                  dc: perms.dc !== undefined ? perms.dc : true,
                  dovai: finalDovai
                }
              };
            }
          ...
          if (!migratedRoles.some((r: any) => (r.name || '').toLowerCase().includes('hộ lý'))) {
            const insertIdx = migratedRoles.length > 0 ? migratedRoles.length - 1 : 0;
            migratedRoles.splice(insertIdx, 0, {
              name: 'Hộ lý',
              color: '#eab308',
              desc: 'Chỉ giao nhận đồ vải khoa (không xuất nhập kho)',
        ...
        let loadedUsers = Array.isArray(parsed.USERS) ? [...parsed.USERS] : [...INITIAL_USERS];
        let loadedAccounts = Array.isArray(parsed.ACCOUNTS) ? [...parsed.ACCOUNTS] : [...INITIAL_ACCOUNTS];
        let migratedAccountsChanged = false;

        const adminIdx = loadedAccounts.findIndex(a => (a.username || '').toLowerCase() === 'admin');
        if (adminIdx === -1) {
          loadedAccounts.push({
            username: 'Admin',
          ...
          migratedAccountsChanged = true;
        } else {
          const currentAdmin = loadedAccounts[adminIdx];
          if (currentAdmin.username !== 'Admin' || currentAdmin.password !== 'admin123' || !currentAdmin.isAdmin) {
            loadedAccounts[adminIdx] = {
              ...currentAdmin,
              username: 'Admin',
              password: 'admin123',
              isAdmin: true
        ...
        const rolesMap: Record<number, { role: number, dept: string }> = {
          1: { role: 0, dept: 'Kho trung tâm' },
          2: { role: 1, dept: 'Kho trung tâm' },
          3: { role: 2, dept: 'Khoa Cấp cứu đa khoa' },
          4: { role: 5, dept: 'Khoa Cấp cứu đa khoa' },
          5: { role: 6, dept: 'Xưởng giặt Cty' },
          6: { role: 3, dept: 'Tất cả' }
        };

        for (let i = 1; i <= 6; i++) {
          const uName = `test${i}`;
          const name = `Test ${i}`;
          const email = `test${i}@hospital.vn`;

          let uIdx = loadedUsers.findIndex(u => (u.email || '').toLowerCase() === email);
          if (uIdx === -1) {
            const rConfig = rolesMap[i];
            loadedUsers.push({
              name,
              email,
              role: rConfig.role,
            ...
            uIdx = loadedUsers.length - 1;
            migratedAccountsChanged = true;
          }

          const accIdx = loadedAccounts.findIndex(a => (a.username || '').toLowerCase() === uName);
          if (accIdx === -1) {
            loadedAccounts.push({
              username: uName,
              email,
            ...
            const currentAcc = loadedAccounts[accIdx];
            if (currentAcc.password !== '123456' || currentAcc.userIdx !== uIdx) {
              loadedAccounts[accIdx] = {
                ...currentAcc,
                password: '123456',
                userIdx: uIdx
              };
        ...
        if (Array.isArray(parsed.PENDING_REGISTRATIONS)) setPendingRegs(parsed.PENDING_REGISTRATIONS);
        setWardDeliverySlips(Array.isArray(parsed.wardDeliverySlips) ? parsed.wardDeliverySlips : INITIAL_WARD_DELIVERY_SLIPS);
        setLaundryDispatches(Array.isArray(parsed.laundryDispatches) ? parsed.laundryDispatches : INITIAL_LAUNDRY_DISPATCHES);
        setTemporaryCleanStore(parsed.temporaryCleanStore || {});
        setTemporaryDirtyStore(parsed.temporaryDirtyStore || {});
        setTemporaryCompanyDirtyStore(parsed.temporaryCompanyDirtyStore || {});
      } else {
        // First-time loading fallback
